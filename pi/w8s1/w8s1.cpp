...
const float DISP_FULL_DIST    = 12000;
const float DISP_DEFAULT_DIST = 6000;
...
u_result capture_and_display(RPlidarDriver * drv, const char* fname)
{
    ...
    if (IS_OK(ans) || ans == RESULT_OPERATION_TIMEOUT) {
        ...
        for (int pos = 0; pos < (int)count ; ++pos) {
            ...
            // to (X,Y) coordinate
            float endptX = 300, endptY = 300;
            
            
            if (dist) {
                endptX += (300 * dist / DISP_DEFAULT_DIST) * sin(rad);
                endptY += (300 * dist / DISP_DEFAULT_DIST) * cos(rad);
            }
