...
void startSerial(const char *portName, int baudRate, int byteSize, char parity, int stopBits, int maxAttempts) {
	...
	while(_fd < 0 && attempt < maxAttempts) {
	  printf("[PI] ATTEMPTING TO CONNECT TO SERIAL. ATTEMPT # %d of %d.\n", ++attempt, maxAttempts);
   	_fd=open(portName, O_RDWR | O_NOCTTY | O_NDELAY);

		if(_fd < 0) {
		  printf("[PI] FAILED. TRYING AGAIN IN 5 SECONDS\n");
		  sleep(5);
		}
	}
    
    if(_fd < 0) perror("[PI] GIVING UP. Unable to open serial port.");
    else {
        fcntl(_fd, F_SETFL, 0);
